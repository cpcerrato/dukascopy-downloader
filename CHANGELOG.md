...
- CLI improvements: prints a run summary (instrument/timeframe/range/TZ), shows a single spinner-style progress line instead of per-file logs, and reports download/generation/total timings plus a download summary.
- Added `DownloadSummary` to surface counts/durations.
- Integration coverage for tick UTC vs TZ parity, Unix-ms defaults, and s1-to-candle aggregation; E2E/Integration tests updated for export paths.
